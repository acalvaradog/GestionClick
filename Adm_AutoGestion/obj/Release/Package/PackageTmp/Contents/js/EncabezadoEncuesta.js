...
function MostrarDatos() {
    ...
    var area = document.getElementById("Preguntas_Id").value;
...
function RepuestasxPregunta() {
    //var data = JSON.stringify({ 'area': 'area', 'fechainicial': '1', 'fechafinal': '1' });
    var empresa = document.getElementById("Preguntas_Id").value;

    var fechainicial = document.getElementById("txt_fechainicial").value;
    var fechafinal = document.getElementById("txt_fechafinal").value;
    try {
        $.ajax({
            url: "RepuestasXPreguntas",
            data: { 'FechaIni': fechainicial, 'FechaFin': fechafinal, 'Pregunta': empresa },
            type: "post",
            success: function (e) {
                if (e != undefined || e != null) {
                    ...
                    for (var i = 0; i < e.length; i++) {
                        ...
                        data.addRow([e[i].Documento, e[i].Nombres, e[i].Area, e[i].Telefono, e[i].Correo, e[i].Cargo, e[i].Empresa]);
                    }
                    ...
                    //var components = [{ type: 'csv', datasource: e },
                    //];
...
function GraficasNoRealizadas() {
    ...
    try {
        $.ajax({
            ...
            data: { 'area': area, 'fechainicial': fechainicial, 'fechafinal': fechafinal, 'empresa': empresa },
            type: "post",
            success: function (e) {
                if (e != undefined || e != null) {
                    var data = new google.visualization.DataTable();
...
function MostrarDatosSinContestar() {
    ...
    if ( fechainicial == '' || fechafinal == '') {
        ...
        GraficasSinContestar();
        GraficasSinEncuesta();
        GraficaColoresGlobal();
    }
...
function GraficasSinEncuesta() {
    ...
    try {
        $.ajax({
            url: "DatosSinEncuesta",
            data: { 'fechainicial': fechainicial, 'fechafinal': fechafinal, 'empresa': empresa },
            type: "post",
            success: function (e) {
                if (e != undefined || e != null) {
                    var data = new google.visualization.DataTable();
                    ...
                    for (var i = 0; i < e.length; i++) {
                        ...
                        data.addRow([e[i].Area, e[i].Cargo, e[i].Nombres]);
                    }
...
function GraficaColoresGlobal() {

    var empresa = document.getElementById("Empresa").value;
    try {
        $.ajax({
            url: "TotalColoresGlobal",
